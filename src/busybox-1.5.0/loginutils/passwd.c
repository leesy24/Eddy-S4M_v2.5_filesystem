...
static char* new_password(const struct passwd *pw, uid_t myuid, int algo, char *pass)
{
...
//		printf("Changing password for %s\n", name);
		...
		newp = new_password(pw, myuid, opt & STATE_ALGO_md5, argv[1]);
		if (!newp) {
			logmode = LOGMODE_STDIO;
