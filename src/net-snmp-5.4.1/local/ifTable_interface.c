...
static void _ifTable_container_init(
    ifTable_interface_ctx *if_ctx);
static void _ifTable_container_shutdown(
    ifTable_interface_ctx *if_ctx);
                            ...
                            netsnmp_request_info *requests)
{
    ...
    DEBUGMSGTL(("internal:ifTable:_mfd_ifTable_pre_request",
                "called\n"));
                             ...
                             netsnmp_request_info *requests)
{
    ...
    DEBUGMSGTL(("internal:ifTable:_mfd_ifTable_post_request",
                "called\n"));
...
rc = ifSpeed_get(rowreq_ctx, (u_long *)var->val.string );
    ...
    var->type = ASN_OCTET_STR;
rc = ifPhysAddress_get(rowreq_ctx, (char **)&var->val.string, &var->val_len );
        break;
...
rc = ifSpecific_get(rowreq_ctx, (oid **)&var->val.string, &var->val_len );
        break;
                         ...
                         netsnmp_variable_list *var, int column )
{
    ...
    rc = netsnmp_check_vb_type_and_size( var, ASN_INTEGER,
        sizeof( rowreq_ctx->data.ifAdminStatus ) );
 ...
 && ( *var->val.integer != IFADMINSTATUS_TESTING )
    ...
    if(SNMPERR_SUCCESS != rc) {
        ...
        rc = ifAdminStatus_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifAdminStatus_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
                         ...
                         netsnmp_request_info *requests)
{
    ...
    for(;requests; requests = requests->next) {
        ...
        rc = _ifTable_check_column(rowreq_ctx, requests->requestvb, tri->colnum);
        if(rc) {
            netsnmp_request_set_error(requests, SNMP_VALIDATE_ERR(rc));
            break;
        }
...
_ifTable_undo_setup_column( ifTable_rowreq_ctx *rowreq_ctx, int column )
{
    ...
    case COLUMN_IFADMINSTATUS:
     ...
     default:
         snmp_log(LOG_ERR,"unknown column %d in _ifTable_undo_setup_column\n", column);
         break;
    }
                         ...
                         netsnmp_request_info *requests)
{
    ...
    if (rowreq_ctx->rowreq_flags & MFD_ROW_DIRTY) {
        snmp_log(LOG_WARNING, "ifTable row dirty flag still set after undo_commit\n");
                         ...
                         netsnmp_request_info *requests)
{
    ...
    DEBUGMSGTL(("internal:ifTable:_mfd_ifTable_irreversible:commit","called\n"));
